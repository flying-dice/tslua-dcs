...
-- 
-- @returns A comma-separated string containing the names of all
-- helicopter groups on the BLUE side.
____exports.getHeloGroupNames = function()
    local groups = coalition.getGroups(coalition.side.BLUE, Group.Category.HELICOPTER)
    return __TS__ArrayJoin(
        __TS__ArrayMap(
            groups,
            function(____, group) return group:getName() end
        ),
        ","
    )
end
...
--[[ Generated with https://github.com/TypeScriptToLua/TypeScriptToLua ]]
local ____exports = {}
local ____getHeloGroupNames = require("getHeloGroupNames")
local getHeloGroupNames = ____getHeloGroupNames.getHeloGroupNames
env.info("GROUP_NAMES: " .. getHeloGroupNames(nil))
_G.MK = {_VERSION = "0.1.0", getHeloGroupNames = getHeloGroupNames}
return ____exports
 end,
}
